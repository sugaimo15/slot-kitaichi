...
export default function CalculatorClient({ machines, initialSlug }: Props) {
  const [selectedSlug, setSelectedSlug] = useState(initialSlug ?? machines[0]?.slug ?? "");
  const [setting, setSetting] = useState<number>(6);
  const [spins, setSpins] = useState<number>(700);
  const [hours, setHours] = useState<number>(4);
  const [coinRate, setCoinRate] = useState<number>(3);
  ...
  const result = useMemo(() => {
    ...
    const totalEV = Math.round(diff * coinRate);
  ...
  }, [machine, setting, spins, hours, coinRate]);
      ...
      {result && machine && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          <div className="text-xs text-slate-400 space-y-0.5">
            <p>総回転数: {result.totalSpins.toLocaleString()}回転 / 投入コイン: {result.coinsIn.toLocaleString()}枚 / 払出コイン: {result.coinsOut.toLocaleString()}枚</p>
