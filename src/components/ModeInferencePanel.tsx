...
export default function ModeInferencePanel({ config }: Props) {
  ...
  const recommendation = useMemo(() => {
    ...
    if (tsujoCProb >= 40) return { label: "通常C濃厚。最大残り数周期でAT確定圈", color: "text-green-700", bg: "bg-green-50 border-green-200" };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {totalTransitions > 0 && (
          <div className="sm:col-span-2 bg-slate-50 rounded-lg px-3 py-2 text-xs text-slate-500">
            昇格抗せん合計: <span className="font-bold text-slate-700">{totalTransitions}回</span>
